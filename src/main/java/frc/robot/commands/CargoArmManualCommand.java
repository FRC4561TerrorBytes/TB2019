...
public class CargoArmManualCommand extends Command {
  ...
  protected void execute() {
    // squares the input from the xbox left joystick to make the cargo arm movement
    // slower and more precise, but keeps the sign the same as the original input
    Robot.cargoArmSubsystem.armManual(
        (-Math.copySign(Math.pow(RobotMap.GAME_PAD.getY(Hand.kLeft), 2), RobotMap.GAME_PAD.getY(Hand.kLeft))) / 2);
    // reset the encoder value to the correct position when the encoder is pressed
